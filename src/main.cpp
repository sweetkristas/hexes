...
void draw_ui(const glm::mat4& ortho_camera)
{
	...
	auto r = graphics::render::text::quick_draw(0, 600-40, ss1.str(), "Tauri-Regular.ttf", 14, graphics::color(1.0f, 1.0f, 0.5f));
	ss2 << "Frame process time (uS): " << std::fixed << frame_processing_time;
	graphics::render::text::quick_draw(0, r.y2(), ss2.str(), "Tauri-Regular.ttf", 14, graphics::color(1.0f, 1.0f, 0.5f));
}
...
int main(int argc, char* argv[])
{
	...
	try {
		...
		glm::mat4 scene_camera = glm::perspective(45.0f / 180.0f * static_cast<float>(M_PI), static_cast<float>(width) / static_cast<float>(height), 0.01f, 100.0f);
